...
if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['tan']))
{
	$ga = new PHPGangsta_GoogleAuthenticator();
    $username = $_POST['username'];
    $password = $_POST['password'];
    $tan = $_POST['tan'];
    $fetchuser = mysqli_query($sql, "SELECT * FROM users WHERE name = '" . $username . "'");
    $row = mysqli_fetch_array($fetchuser);
	$userid = $row['id'];
	$secretcode = $row['g_key'];
	$oneCode = $ga->getCode($secretcode);
    $login = $user->verifyLogin($username, $password);
	if ($oneCode == $tan) {

            $checkResult = $ga->verifyCode($secretcode, $oneCode, 2);
    ...
    if($login != false)
    {
        ...
        $user->setUserId($login->id);
        $dbid = $user->getUserInfo()->id;
    ...
    <div class="form-wrap">
        <form method="post">
            <div class="input-wrap">
                ...
                <input name="username" type="name" placeholder="Name" required spellcheck="false" autofocus>
            </div>
            <div class="input-wrap">
            ...
            <div class="input-wrap">
                ...
                <input name="tan" type="password" placeholder="TAN" required>
            </div>
